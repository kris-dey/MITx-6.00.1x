...
def getWordScore(word, n):
    ...
    pt = 0
    for char in word:
        pt += SCRABBLE_LETTER_VALUES[char]
    ...
    while calculateHandlen(hand) > 0:                                                     # As long as there are still letters left in the hand:
        ...
        if word == '.':                                                             # If the input is a single period:
            ...
            if not isValidWord(word, hand, wordList):                                              # If the word is not valid:
                ...
                total += wordScore
                print( '"' + word + '"' + ' earned ' + str(wordScore) + ' points. Total: ' + str(total))                                                                # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                
                hand = updateHand(hand, word)                                                                 # Update the hand 
                

    if word == '.':                                                             # Game is over (user entered a '.' or ran out of letters), so tell user the total score
