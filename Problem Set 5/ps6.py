...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for c in lC:
            c2 = (ord(c) - 97 + shift) % 26 + 97
            d[c] = chr(c2)
            
        for c in uC:
            c2 = (ord(c) - 65 + shift) % 26 + 65
            d[c] = chr(c2)
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift in range(26):
            cW = 0
            w2 = self.apply_shift(shift)
            messageString = w2.split(" ")
            for w in messageString:
        ...
        message = "".join(self.apply_shift(bestShift))
        
        return bestShift, message
            

def decrypt_story():
    story = get_story_string()
    s = CiphertextMessage(story)
